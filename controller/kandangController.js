...
module.exports = {
    ...
    filterRecordByDate: (req, res) => {
        var data = req.body
        console.log(data)
        var sql = ''
        if(data.typeFilter === 1) {
            sql = `	SELECT * FROM kandang.days_record_report WHERE DATE(tanggal) BETWEEN '${data.date}' AND '${data.date2}' 
            AND fid_owner = ${req.logedUser} AND fid_location = ${data.id_location} AND fid_unit = ${data.id_unit} AND fid_rows = ${data.id_rows};`
        } else {    
            sql =  `SELECT * FROM kandang.days_record_report WHERE DATE(tanggal) = '${data.date}' 
            AND fid_owner = ${req.logedUser} AND fid_location = ${data.id_location} AND fid_unit = ${data.id_unit} AND fid_rows = ${data.id_rows};`
        }
